...
/** HTML-escape user-supplied strings before interpolating into print/export HTML. */
...
export function printScenesHtml(
  ...
  const renderSceneHtml = (s: Scene) => {
    ...
    return `<div class="scene" style="border-left-color:${color}"><div class="sh"><span class="sn">Scene ${String(s.sceneNumber).padStart(2,'0')}</span><span style="color:${color};font-weight:700">Act ${s.act}</span><span class="sf">${esc(s.keyFunction||'')}</span></div><h3 class="st">${esc(s.title)}</h3><div class="eb"><span class="el">CELSIUS TENSION INDEX™</span><div class="bar"><div class="fill" style="width:${Math.max(s.energyLevel,5)}%;background:${energyColor}"></div></div><span class="ev">${s.energyLevel}°C</span></div><div class="grid"><div><b class="fl">Purpose</b><p>${esc(s.purpose||'—')}</p></div><div><b class="fl">Summary</b><p>${esc(s.summary||'—')}</p></div><div><b class="fl">Tone</b><p>${esc(s.tone||'—')}</p></div><div><b class="fl">Visual Signature</b><p>${esc(s.visualSignature||'—')}</p></div><div><b class="fl">Conflict (Internal)</b><p>${esc(s.conflictInternal||'—')}</p></div><div><b class="fl">Conflict (External)</b><p>${esc(s.conflictExternal||'—')}</p></div></div><div class="ca">Builds From: ${esc(s.buildsFrom||'—')} → <b>Scene ${s.sceneNumber}</b> → Sets Up: ${esc(s.setsUp||'—')}</div><div class="dm">Dialogue: ${esc(s.dialogueDensity||'—')} · Est. Lines: ${esc(s.estimatedDialogueLines||'—')}</div>${isComic ? `<div class="dm" style="color:#a855f7">Panels: ${s.panelCount||4} · Layout: ${esc((s.pageLayout||'standard-grid').replace(/-/g,' '))}${s.isSplashPage ? ' · <b>SPLASH PAGE</b>' : ''}</div>` : ''}</div>`;
  };

  const scenesHtml = groupedByEpisode.map(g => {
    const epHeader = isMultiUnit ? `<h2 class="ep-header">${esc(unitLabel)} ${g.episode} — ${g.scenes.length} Scenes</h2>` : '';
    return epHeader + g.scenes.map(renderSceneHtml).join('');
  }).join('');

  const subtitle = isMultiUnit
    ? `${esc(mediumLabel)} — ${totalEpisodes} ${esc(unitLabelPlural)} · ${allScenes.length} Total Scenes`
    : `${esc(mediumLabel)} — ${allScenes.length} Scenes`;
